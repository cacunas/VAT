...
segmentationModule::segmentationModule(Datapool *i_data): ModuleInterface(i_data) {
    globalMask = NULL;
    globalValidPoints = 0;
...
bool segmentationModule::setParameters(QDomNode& config){
    QDomNode n, m;
    int intT;
...
//            addParameter("Threshold", getParameterValue(n), "uchar", 0);
        ...
        if( ( n = XmlCommon::getParameterNode("ChromaticSegmentation", config) ).isNull() ) {
            chromaticSegmentationIsActivated=false;

        } else {

            chromaticSegmentationIsActivated = XmlCommon::getParameterValue(n) == "true" ? true : false;
        ...
        if( ( n = XmlCommon::getParameterNode("minSaturation", config) ).isNull() ) {
            minSaturation=25;

        } else {

            minSaturation = XmlCommon::getParameterValue(n).toInt();

        }

        if( ( n = XmlCommon::getParameterNode("maxLight", config) ).isNull() ) {
            maxLight=165;

        } else {

            maxLight = XmlCommon::getParameterValue(n).toInt();
...
bool segmentationModule::updateParameters(){
...
bool segmentationModule::init(){
    if(first == false) {
        ...
        bool retValue =  readImageOnDisk(bgImName,&tmpImage);

        m_data->bgImage=tmpImage;

        //m_data->bgImage = new QImage(bgImName);
        if(m_data->bgImage == NULL || m_data->bgImage->isNull()) {
            AppendToLog("SegmentationModule: Warning: Cannot open background image '" + bgImName + "'. Considering first image in sequence as background.\n");
...
bool segmentationModule::setGlobalZones() {
...
bool segmentationModule::insideInZone(int i, int j) {
...
//i :y img coord, j: x img coord
bool segmentationModule::outsideOutZone(int i, int j) {
...
bool segmentationModule::setGlobalMask() {
...
void segmentationModule::updateBGGlobally(QImage *current) {
...
bool segmentationModule::run(){
    ...
    if(chromaticSegmentationIsActivated){
        SegmentationTool st;

        //TEMP
        st.chromaticSegmentation(m_data->currentImage, m_data->fgImage, m_data->rFgImage, m_data->maskImage,m_mask,minGrade,maxGrade, minSaturation, maxLight);

    }else{
        getDiff(m_data->fgImage, m_data->rFgImage);
    }
...
void segmentationModule::applyAperture(QImage *fg, QImage *fg_c) {
...
void segmentationModule::getDiff(QImage *result, QImage *result_c) {
    if(result == NULL || result_c == NULL || result->isNull() || result_c->isNull())
        return;
    int w = result->width(),
        h = result->height(),
    ...
    if(m_mask) {
        ...
        for(i=0; i<h; i++) {
            for(j=0, k=0; k<w; j+=4, k++) {
                if(mask[i*b2 + k]) {
                    aux = i*b1 + j;
                    d1 = abs(new_bits[aux + 2] - bg_bits[aux + 2]);
                    d2 = abs(new_bits[aux + 1] - bg_bits[aux + 1]);
                    d3 = abs(new_bits[aux] - bg_bits[aux]);
//                    if(d1>T || d2>T || d3>T) {
                    if(d1*d1 + d2*d2 + d3*d3 > T2) {
                        bits[i*b2 + k] = 255;
...
void segmentationModule::updateBG(QImage *current) {
    if(current == NULL)
        return;
    int w,h;
    w = current->width();
...
bool segmentationModule::readImageOnDisk(QString dir,QImage **o_rpOutputImage) {


    cv::Mat opencvImage= cv::imread(dir.toStdString());

    if(opencvImage.channels() != 3) {
        std::cout << "segmentationModule: Error: Expected 3 channels opencv image. Normally a bad Background Image configuration error. Image channels: " << opencvImage.channels() << std::endl;
        AppendToLog("segmentationModule: Error: Expected 3 channels opencv image. Normally a bad Background Image configuration error. Image channels: " + QString::number(opencvImage.channels()));
        return false;
    }
    ...
    **o_rpOutputImage = img->convertToFormat(QImage::Format_ARGB32);
    ...
    if((*o_rpOutputImage)->isNull()) {
        AppendToLog("ReadImageOnDisk: Impossible to load image (load_image returned error for file " + dir );
        return false;
    }
    return true;
}
