#include "regionalSegmentationModule.h"
...
regionalSegmentationModule::regionalSegmentationModule(Datapool *i_data): ModuleInterface(i_data) {
    m_bins = 32;
    w_size = 11;
    m_hullOffset = 1;
    m_K = 2;
...
regionalSegmentationModule::~regionalSegmentationModule() {}
...
bool regionalSegmentationModule::updateParameters(){
...
bool regionalSegmentationModule::init(){
    return true;
}

//Formato ARGB
//b == bits[i]    g == bits[i+1]    r == bits[i+2]    alpha == bits[i+3]
bool regionalSegmentationModule::run(){
    if(m_data->blobs.size() > 0 && m_data->fgImage != NULL && m_data->currentImage != NULL) //Requires foreground and current images
        completeBlobs(m_data->blobs, m_data->fgImage, m_data->currentImage);
    return true;
}
...
void regionalSegmentationModule::getNormalIntersections(cv::Mat &f,
                                   cv::Rect &roi,
                                   std::vector<cv::Point> &hull,
...
void regionalSegmentationModule::setForegroundSegment(cv::Mat &f, cv::Rect &roi,
                                                      std::vector<cv::Point> &hull,
                                                      segment2D<float> &seg,
                                                      float x, float y,
                                                      float dx, float dy,
...
int regionalSegmentationModule::getContourToLineIntersectionIndexed(std::vector<cv::Point> &polygon,
                                         cv::Point3f &rline,
                                         cv::Point2f &r1, cv::Point2f &r2,
                                         int &n1, int &n2) {
...
int regionalSegmentationModule::getContourToLineIntersection(std::vector<cv::Point> &polygon,
                                         cv::Point3f &rline,
                                         cv::Point2f &r1, cv::Point2f &r2,
                                         int *n1, int *n2) {
...
bool regionalSegmentationModule::lineSegmentIntersection(cv::Point3f &line,
...
//a y + b x + c = 0
bool regionalSegmentationModule::getGeneralLineForm(cv::Point2f p1, cv::Point2f p2,
                                                    cv::Point3f &rline) {
...
bool regionalSegmentationModule::getGeneralLineForm(float x, float y,
                                                    float dx, float dy,
                                                    cv::Point3f &rline) {
...
double regionalSegmentationModule::distanceToSegment(int i1, int i2, std::vector<cv::Point> &contour) {
...
bool regionalSegmentationModule::movementFound(cv::Mat f, int wsize, int i0, int j0) {
...
bool regionalSegmentationModule::movementFound(cv::Mat f, int wsize, int i0, int j0, cv::Rect roi) {
    int i, j, diff = wsize/2,
        y1 = f.rows, x1 = f.cols;
...
int regionalSegmentationModule::movementCount(cv::Mat f, int wsize, int i0, int j0, cv::Rect roi) {
